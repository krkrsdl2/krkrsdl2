...
def make_func_stub(rettype, name, arg, type_, prefix, isconst, isstatic):
...
def list_func_stub(prefix, content, type_):
	for match_obj in re.finditer(prefix + r"_METHOD_DEF\(\s*(.*?)\s*,\s*(.*?)\s*,\s*\((.*?)\)\s*\)", content, flags=re.S): # g
		make_func_stub(match_obj.group(1), match_obj.group(2), match_obj.group(3), type_, prefix, False, False)
	for match_obj in re.finditer(prefix + r"_CONST_METHOD_DEF\(\s*(.*?)\s*,\s*(.*?)\s*,\s*\((.*?)\)\s*\)", content, flags=re.S): # g
		make_func_stub(match_obj.group(1), match_obj.group(2), match_obj.group(3), type_, prefix, True, False)
	for match_obj in re.finditer(prefix + r"_STATIC_METHOD_DEF\(\s*(.*?)\s*,\s*(.*?)\s*,\s*\((.*?)\)\s*\)", content, flags=re.S): # g
		make_func_stub(match_obj.group(1), match_obj.group(2), match_obj.group(3), type_, prefix, False, True)
	for match_obj in re.finditer(prefix + r"_STATIC_CONST_METHOD_DEF\(\s*(.*?)\s*,\s*(.*?)\s*,\s*\((.*?)\)\s*\)", content, flags=re.S): # g
		make_func_stub(match_obj.group(1), match_obj.group(2), match_obj.group(3), type_, prefix, True, True)


def make_exp_stub(rettype, name, arg):
	rettype = re.sub(r"\n", r" ", rettype, flags=re.S) # g
	rettype = re.sub(r"\t", r" ", rettype, flags=re.S) # g
	name = re.sub(r"\n", r" ", name, flags=re.S) # g
	name = re.sub(r"\t", r" ", name, flags=re.S) # g
...
def process_exp_stub(file):
	...
	for match_obj in re.finditer(r"TJS_EXP_FUNC_DEF\(\s*(.*?)\s*,\s*(.*?)\s*,\s*\((.*?)\)\s*\)", content, flags=re.S): # g
		make_exp_stub(match_obj.group(1), match_obj.group(2), match_obj.group(3))
...
h_stub = []
list_func_stub("TJS", srch.group(1), "tTJSVariant")
variant = h_stub

srch = re.search(r"\/\*start-of-tTJSVariantOctet\*\/(.*?)\/\*end-of-tTJSVariantOctet\*\/", content, flags=re.S)
h_stub = []
list_func_stub("TJS", srch.group(1), "tTJSVariantOctet")
variantoctet = h_stub
...
h_stub = []
list_func_stub("TJS", srch.group(1), "tTJSString")
string_ = h_stub
...
h_stub = []
list_func_stub("TJS", srch.group(1), "tTJSVariantString")
